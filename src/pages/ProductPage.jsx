...
export default function ProductPage({ product, onBack, onAddToCart, onBuyNow }) {
  ...
  // ── FIX 1: selected color variant ────────────────────
  // `variants` is the array from the admin panel: [{ colorName, colorHex, images, sizes, available }]
  // `colors`   is the old simple array: [{ name, hex }]
  // We support both formats.
  const variants = product.variants || [];
  const hasVariants = variants.length > 0;

  const [selectedVariant, setSelectedVariant] = useState(() => {
    // Auto-pick first available variant, or first variant
    if (hasVariants) {
      return variants.find(v => v.available !== false) || variants[0] || null;
    }
    return null;
  });

  // Derive the image list from the selected variant (if any), else from the product
  const getImages = () => {
  ...
  const handleAdd = () => {
    // Pass along selected variant info
  ...
  // ── Color swatch renderer (supports both formats) ─────
  const renderColorSwatches = () => {
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: isSelected ? "3px solid #1A1816" : "2px solid transparent",
                  outline: isSelected ? "1px solid #1A1816" : "1px solid rgba(26,24,22,0.15)",
                  outlineOffset: "2px",
                ...
                {isSelected && (
                  <span style={{
                    ...
                    <Check size={13} strokeWidth={3}
                      style={{ color: parseInt((v.colorHex||"#ccc").replace("#",""), 16) > 0x888888 ? "#1A1816" : "#FAF9F7" }} />
                  </span>
                )}
              </button>
            );
    ...
    // Legacy: product.colors = [{ name, hex }]
    if (product.colors && product.colors.length > 0) {
      return (
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "24px" }}>
          {product.colors.map((c, i) => (
            <button key={i} title={c.name}
              style={{ width: "28px", height: "28px", borderRadius: "50%", backgroundColor: c.hex || "#ccc", border: "1px solid rgba(26,24,22,0.15)", cursor: "pointer", padding: 0, flexShrink: 0 }}
            />
          ))}
        </div>
      );
  ...
  const selectedColorName = selectedVariant?.colorName || null;
          ...
          <div style={{ paddingTop: "8px" }}>
            ...
            {(hasVariants || (product.colors && product.colors.length > 0)) && (
              <div style={{ marginBottom: "4px" }}>
                <p style={{ fontSize: "8px", fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", color: "#9A9590", marginBottom: "10px" }}>
                  Color{selectedColorName ? <span style={{ color: "#1A1816", fontWeight: 500, textTransform: "none", letterSpacing: 0 }}>: {selectedColorName}</span> : ""}
                </p>
                {renderColorSwatches()}
              </div>
            )}
