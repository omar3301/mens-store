...
function ProductPage({ product, onClose, onAddToCart }) {
  ...
  // When color changes, switch to that color's image index if it matches
  const handleColorChange = (idx) => {
    setSelColorIdx(idx);
    // If each color has a corresponding image, switch to it
    if (imgs && idx < imgs.length) setImgIdx(idx);
  };
...
export default function App() {
  ...
  const openProduct = (p) => { setActiveProduct(p); setPage("product"); window.scrollTo({top:0,behavior:"instant"}); };
      ...
      <>
        ...
        <nav style={{ position:"sticky",top:0,zIndex:700,background:"rgba(12,12,12,0.9)",backdropFilter:"blur(16px)",borderBottom:"1px solid var(--border)",padding:"0 20px" }}>
          <div style={{ maxWidth:1280,margin:"0 auto",height:56,display:"flex",alignItems:"center",justifyContent:"space-between" }}>
            ...
            <div style={{ fontFamily:"'Syne',sans-serif",fontSize:15,fontWeight:800,letterSpacing:"0.18em" }}>SOLA</div>
